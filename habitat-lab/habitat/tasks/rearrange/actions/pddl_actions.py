...
class PddlApplyAction(ArticulatedAgentAction):
    ...
    def step(self, *args, **kwargs):
        ...
        # Toggle RVO only when a PDDL primitive is explicitly selected this step.
        # Oracle skills often run for many steps without writing into the PDDL
        # action vector; in that case leave RVO state unchanged (e.g. after
        # reset's enable_rvo). Disable only for explicit non-social actions
        # (back_off, wait, etc.).
        try:
                ...
                if task_info is not None and task_info.get("task") == "NavToObjSocialTask-v0":
                    if hasattr(self._task, "enable_rvo"):
                        try:
                            ...
                            pass
                elif hasattr(self._task, "disable_rvo"):
                    try:
