...
class Hallucinate():
    ...
    def read_data(self, folder, noise_counter):
        ...
        self.objs[0].base_rot = heading[0]+np.pi
    ...
    def get_min_dist(self, traj1, traj2):
        ...
        point1 = traj1[-1]
        point2 = traj2[-1]
        for ([point1[0], point1[1]], [point2[0], point2[1]]) in (traj1, traj2):
    ...
    def get_trajectory(self, folder, data_dir, noise_counter):
        ...
        # threshold_dist = self.get_min_dist(robot_past_at_crossing, human_past_at_crossing)
        threshold_dist = 0.7
        ...
        while not found_path or points_outside:
            ...
            trial_counter += 1
            new_robot_traj = traj_interp(np.array(new_robot_traj))
            diff = sampled_new_pos - human_pos_world.T
            diff = diff[0]
            diff[1] = 0
            dist_from_human = np.linalg.norm(diff)
            # dist_from_human = np.linalg.norm((sampled_new_pos - human_pos_world)[[0,2]])
            if dist_from_human < threshold_dist:
                points_outside = True
